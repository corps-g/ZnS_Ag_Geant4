...
:fPhysiWorld(NULL)
{
	...
	// the light guide was modeled as sector with central angle of 160 degree
	// here we compute the diameter of the full cylinder
...
G4VPhysicalVolume* WLSDetectorConstruction::ConstructDetector()
{
	...
	// World
	//--------------------------------------------------
	// extend the length to allow a point Cf252 source 2-cm away from the detector
	G4VSolid* solidWorld = new G4Box("World", diameter, diameter, lenz + 2.0 * cm);
