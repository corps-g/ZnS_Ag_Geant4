...
WLSMaterials::WLSMaterials(G4double len)
{
...
WLSMaterials* WLSMaterials::GetInstance(G4double len)
{
  if (fInstance == 0)
    {
      fInstance = new WLSMaterials(len);
    }
  return fInstance;
}
...
void WLSMaterials::CreateMaterials()
{
  ...
  G4double fZnSAgAbsLen[] = {absLen, absLen, absLen, absLen};
  G4cout << "zsa abs length = " << absLen / um << "um\n";
  n = 4;
