...
B1RunAction::B1RunAction(G4int n)
: G4UserRunAction(){
	nUnit = n;
}
...
void B1RunAction::EndOfRunAction(const G4Run* run)
{
	...
	if (IsMaster()) {
		...
		G4double alphaEnergy = b3Run -> GetAlphaDepoE();
		G4double alphaEnergy2 = b3Run -> GetAlphaDepoE2();
		G4double ionEnergy = b3Run -> GetIonDepoE();
		G4double ionEnergy2 = b3Run -> GetIonDepoE2();
		G4double elecEnergy = b3Run -> GetElecDepoE();
		G4double elecEnergy2 = b3Run -> GetElecDepoE2();

		vecG4double total, proton, alpha, ion, elec;
		total = cal_ave(totalEnergy, totalEnergy2, nofEvents);
		proton = cal_ave(protonEnergy, protonEnergy2, nofEvents);
		alpha = cal_ave(alphaEnergy, alphaEnergy2, nofEvents);
		ion = cal_ave(ionEnergy, ionEnergy2, nofEvents);
		elec = cal_ave(elecEnergy, elecEnergy2, nofEvents);

		G4cout
		<< "Ave. total deposited energy in ZnS(Ag), relative error = "
		<< total[0] / MeV << " MeV,  " << total[1] << "\n"
		<< "Ave. proton deposited energy in ZnS(Ag), relative error = "
		<< proton[0] / MeV << " MeV, " << proton[1] << "\n"
		<< "ave. alpha deposited energy in ZnS(Ag), relative error = "
		<< alpha[0] / MeV << " Mev, " << alpha[1] << "\n"
		<< "ave. ion deposited energy in ZnS(Ag), relative error = "
		<< ion[0] / MeV << " Mev, " << ion[1] << "\n"
		<< "ave. elec deposited energy in ZnS(Ag), relative error = "
		<< elec[0] / MeV << " Mev, " << elec[1] << G4endl;
	}
	else {
		G4cout
		<< G4endl
