  ...
  elecEnergy2(0.)
{
	...
	alphaEnergyID = SDM -> GetCollectionID("ZnSAg/edepAlpha");
	ionEnergyID = SDM -> GetCollectionID("ZnSAg/edepIon");
	elecEnergyID = SDM -> GetCollectionID("ZnSAg/edepElec");
}
...
void B3bRun::RecordEvent(const G4Event* event)
{
	...
	G4THitsMap<G4double>* evtMapAlpha =
			static_cast<G4THitsMap<G4double>*>(HCE->GetHC(alphaEnergyID));
	for (itr = evtMapAlpha -> GetMap() -> begin();
			itr != evtMapAlpha -> GetMap()->end(); itr++) {
		G4double tmp = *(itr -> second);
		alphaEnergy += tmp;
		alphaEnergy2 += tmp * tmp;
	}

	G4THitsMap<G4double>* evtMapIon =
			static_cast<G4THitsMap<G4double>*>(HCE->GetHC(ionEnergyID));
	for (itr = evtMapIon -> GetMap() -> begin();
			itr != evtMapIon -> GetMap()->end(); itr++) {
		G4double tmp = *(itr -> second);
		ionEnergy += tmp;
		ionEnergy2 += tmp * tmp;
	}

	G4THitsMap<G4double>* evtMapElec =
			static_cast<G4THitsMap<G4double>*>(HCE->GetHC(elecEnergyID));
	for (itr = evtMapElec -> GetMap() -> begin();
			itr != evtMapElec -> GetMap()->end(); itr++) {
		G4double tmp = *(itr -> second);
		elecEnergy += tmp;
		elecEnergy2 += tmp * tmp;
	}
...
void B3bRun::Merge(const G4Run* aRun)
{
	...
	alphaEnergy += localRun -> alphaEnergy;
	alphaEnergy2 += localRun -> alphaEnergy2;
	elecEnergy += localRun -> elecEnergy;
	elecEnergy2 += localRun -> elecEnergy2;
	ionEnergy += localRun -> ionEnergy;
	ionEnergy2 += localRun -> ionEnergy2;
	G4Run::Merge(aRun);
} 
