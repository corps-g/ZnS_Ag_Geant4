...
G4VPhysicalVolume* WLSDetectorConstruction::ConstructDetector()
{
	...
	// World
	//--------------------------------------------------
	// expand the world to allow a Cf252 source 2-cm away in z from detector
	G4VSolid* solidWorld = new G4Box("World", lenx, leny, lenz + 2.0 * cm);
	...
	for(G4int i=0; i < nUnit; i++){
		// z-minimum plane of the detector was placed at z = 0
