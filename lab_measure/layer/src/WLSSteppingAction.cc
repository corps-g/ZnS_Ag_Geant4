...
WLSSteppingAction::WLSSteppingAction(B1EventAction* eventAction, G4double l1,
		G4double l2, G4int n)
...
void WLSSteppingAction::UserSteppingAction(const G4Step* theStep)
{
	...
	case FresnelRefraction:
		if (thePostPVname == "World") {
			...
			if (locz > 0.){
				// Not emitted from front window surface
				fEventAction -> IncTotal();
			}
