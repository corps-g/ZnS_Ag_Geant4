...
def submit(dirName, lenz, wr, fname):
    # Submit file
...
if __name__ == '__main__':
    ...
    wr = 0.12
    lenz = 5.
    gperEvent = [1] + range(10, 101, 10)
    for gpe in gperEvent:
        fname = 'g%i' % gpe
        dirg = writer(lenz, wr, 'gamma', True, True, history*10, gpe)
        submit(dirg, lenz, wr, fname)
    dirn = writer(lenz, wr, 'neutron', True, True, history, 10)
    submit(dirn, lenz, wr, 'n1_gamma_rej')
