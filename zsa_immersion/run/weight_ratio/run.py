...
def submit(dirName, lenz, wr):
    # Submit file
    fname = 'wr%.2f.qsub'%wr
    os.system('cp ~/submit/submit.qsub %s/%s'%(dirName, fname))
    with open(dirName + '/%s'%fname, 'a') as f:
        f.write('\n%s/build/exampleB1 run.mac %.6f %.6f > out.txt' % \
                (os.path.abspath('.'), lenz, wr))
    # Submit job
    os.system('cd ' + dirName + ' && qsub %s'%fname)
...
#     b = np.arange(.4, .7, .1)
#     wrs = np.append(a, b)
    # run weight ratio = 0.5, 0.6 on Beocat
    wrs = [0.5, 0.6]
    gperEvent = 10
    for wr in wrs:
        ...
        submit(dirn, lenz, wr)
        if abs(wr - 0.6) < 1e-6:
            # results of the wr = 0.5 gamma case already exist 
