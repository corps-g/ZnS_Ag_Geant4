...
void B1RunAction::EndOfRunAction(const G4Run* run)
{
	...
	if (IsMaster()) {
		...
		G4double e_energy = b3Run -> GetElecDepoE();
		G4double e_energy2 = b3Run -> GetElecDepoE2();

		vecG4double total, proton, elec;
		total = cal_ave(totalEnergy, totalEnergy2, nofEvents);
		proton = cal_ave(protonEnergy, protonEnergy2, nofEvents);
		elec = cal_ave(e_energy, e_energy2, nofEvents);
